...
function removePendingLocalPhoto(photoId: string): void {
	...
	pendingLocalPhotos = pendingLocalPhotos.filter((item) => item.id !== photoId);
}

function removePendingExternalPhoto(photoId: string): void {
	pendingExternalPhotos = pendingExternalPhotos.filter((item) => item.id !== photoId);
}
...
function queueLocalFiles(files: File[]): void {
	...
	if (added > 0) {
		flash(`已加入待上传 ${added} 张，跳过 ${skipped} 张${firstReason ? `（${firstReason}）` : ""}`);
		return;
	}
...
async function ensureUploadPermissions(): Promise<void> {
	...
	if (!response.ok || !data?.permissions) {
		throw new Error(`无法校验上传权限：${getApiMessage(data, response.statusText || "未知错误")}`);
	}
...
async function saveAlbum(): Promise<void> {
	...
	try {
		...
		for (let i = 0; i < localQueue.length; i++) {
			...
			if (!uploadRes.ok) {
				throw new Error(`上传图片失败：${getApiMessage(uploadData, uploadRes.statusText || "未知错误")}`);
			}
			...
			setSaveProgress(doneSteps, totalSteps, `写入相册 ${i + 1}/${localQueue.length}...`);
			...
			if (!photoRes.ok) {
				throw new Error(`写入相册失败：${getApiMessage(photoData, photoRes.statusText || "未知错误")}`);
			}
			...
			pendingLocalPhotos = pendingLocalPhotos.filter((item) => item.id !== pending.id);
		}
		...
		for (let i = 0; i < externalQueue.length; i++) {
			...
			setSaveProgress(doneSteps, totalSteps, `写入外链图片 ${i + 1}/${externalQueue.length}...`);
			const { response: photoRes, data: photoData } = await api(
				`/api/v1/me/albums/${albumId}/photos`,
				{
					method: "POST",
			...
			if (!photoRes.ok) {
				throw new Error(`写入外链图片失败：${getApiMessage(photoData, photoRes.statusText || "未知错误")}`);
			}
			const createdPhoto = parsePhotoItem(photoData?.item);
			if (!createdPhoto) {
			...
			currentPhotoCount += 1;
			doneSteps += 1;
			pendingExternalPhotos = pendingExternalPhotos.filter((item) => item.id !== pending.id);
		}
...
onDestroy(() => {
	...
	<section class="card-base p-6 rounded-[var(--radius-large)] space-y-3 text-90">
		{#if editing}
			<div class="space-y-4">
				<div>
					<label class="block text-sm font-medium mb-1 text-75" for="ed-title">标题</label>
					<input id="ed-title" bind:value={mTitle} class="w-full h-10 px-3 rounded-lg border border-[var(--line-divider)] bg-[var(--card-bg)] text-90 focus:outline-none focus:ring-2 focus:ring-[var(--primary)] transition" />
					<span class="text-xs mt-1 block {weightedCharLength(mTitle) > ALBUM_TITLE_MAX ? 'text-red-500' : 'text-50'}">{weightedCharLength(mTitle)} / {ALBUM_TITLE_MAX}</span>
				</div>
				<div>
					<label class="block text-sm font-medium mb-1 text-75" for="ed-desc">描述</label>
					<textarea id="ed-desc" bind:value={mDescription} rows="3" class="w-full px-3 py-2 rounded-lg border border-[var(--line-divider)] bg-[var(--card-bg)] text-90 focus:outline-none focus:ring-2 focus:ring-[var(--primary)] transition resize-y"></textarea>
				</div>
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						<label class="block text-sm font-medium mb-1 text-75" for="ed-cat">分类</label>
						<input id="ed-cat" bind:value={mCategory} placeholder="例：旅行" class="w-full h-10 px-3 rounded-lg border border-[var(--line-divider)] bg-[var(--card-bg)] text-90 focus:outline-none focus:ring-2 focus:ring-[var(--primary)] transition" />
					</div>
					<div>
						<label class="block text-sm font-medium mb-1 text-75" for="ed-tags">标签（逗号分隔）</label>
						<input id="ed-tags" bind:value={mTags} placeholder="风景, 街拍" class="w-full h-10 px-3 rounded-lg border border-[var(--line-divider)] bg-[var(--card-bg)] text-90 focus:outline-none focus:ring-2 focus:ring-[var(--primary)] transition" />
					</div>
					<div>
						<label class="block text-sm font-medium mb-1 text-75" for="ed-date">日期</label>
						<input
							id="ed-date"
							type="date"
							bind:value={mDate}
					...
					<div>
						<label class="block text-sm font-medium mb-1 text-75" for="ed-loc">地点</label>
						<input id="ed-loc" bind:value={mLocation} placeholder="例：东京" class="w-full h-10 px-3 rounded-lg border border-[var(--line-divider)] bg-[var(--card-bg)] text-90 focus:outline-none focus:ring-2 focus:ring-[var(--primary)] transition" />
					</div>
				</div>
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
					<div>
						<label class="block text-sm font-medium mb-1 text-75" for="ed-layout">布局</label>
						<select id="ed-layout" bind:value={mLayout} class="w-full h-10 px-3 rounded-lg border border-[var(--line-divider)] bg-[var(--card-bg)] text-90 focus:outline-none focus:ring-2 focus:ring-[var(--primary)] transition cursor-pointer">
							<option value="grid">网格</option>
							<option value="masonry">瀑布流</option>
						</select>
					</div>
					<div>
						<label class="block text-sm font-medium mb-1 text-75" for="ed-status">状态</label>
					...
					<div class="flex flex-col justify-end gap-2">
						<label class="flex items-center gap-3 text-75 cursor-pointer select-none">
							<input type="checkbox" bind:checked={mIsPublic} class="toggle-checkbox" />
							<span class="toggle-track"><span class="toggle-knob"></span></span>
							公开此相册
						</label>
						<label class="flex items-center gap-3 text-75 cursor-pointer select-none">
							<input type="checkbox" bind:checked={mShowOnProfile} class="toggle-checkbox" />
							<span class="toggle-track"><span class="toggle-knob"></span></span>
							在主页展示
						</label>
					</div>
		...
		<section class="card-base p-5 rounded-[var(--radius-large)] space-y-3 text-90">
			...
			<div class="flex flex-wrap items-end gap-3">
				<label class="px-4 h-9 rounded-lg text-sm font-medium cursor-pointer bg-[var(--primary)] text-white hover:opacity-90 transition flex items-center gap-1.5 {saving ? 'opacity-50 pointer-events-none' : ''}">
					...
					选择图片
					<input type="file" accept="image/*" multiple class="hidden" on:change={handleFileUpload} disabled={saving} />
				</label>
				...
				<div class="space-y-2">
					...
					<div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
						{#each pendingLocalPhotos as item (item.id)}
							<div class="relative rounded-lg overflow-hidden border border-[var(--line-divider)] bg-[var(--card-bg)]">
								<img src={item.previewUrl} alt={item.file.name} class="w-full h-24 object-cover" loading="lazy" />
								<button
				...
				<div class="space-y-2">
					...
					<div class="space-y-1">
						{#each pendingExternalPhotos as item (item.id)}
							<div class="flex items-center gap-2 rounded-lg border border-[var(--line-divider)] px-2 py-1.5">
	...
	{#if mPhotos.length === 0}
		<div class="card-base p-8 rounded-[var(--radius-large)] text-70">
			{editing ? "尚未添加照片。可先加入待上传队列，再点击“保存相册”。" : "该相册暂无可展示照片。"}
		</div>
	{:else}
		<div class={mLayout === "masonry" ? "columns-2 md:columns-3 gap-3" : "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3"}>
			{#each mPhotos as photo, index (photo.id)}
				<figure
					class="rounded-xl overflow-hidden border border-[var(--line-divider)] bg-[var(--card-bg)] relative group {mLayout === 'masonry' ? 'mb-3 break-inside-avoid' : ''}"
				...
				>
					{#if photoSrc(photo)}
						<img src={photoSrc(photo)} alt={photo.title || "album photo"} class="w-full h-auto object-cover" loading="lazy" />
					{/if}
					{#if !editing && (photo.title || photo.description)}
						<figcaption class="p-3 space-y-1 text-90">
							{#if photo.title}<div class="text-sm font-medium">{photo.title}</div>{/if}
							{#if photo.description}<div class="text-xs text-60">{photo.description}</div>{/if}
						</figcaption>
					{/if}
					...
					{#if editing}
						<div class="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
							<div class="flex items-center gap-2">
								<button
									...
									title="设为封面"
								>
									<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>
								</button>
								<button
						...
						<div class="absolute top-2 left-2 w-6 h-6 rounded bg-black/50 text-white flex items-center justify-center cursor-grab opacity-0 group-hover:opacity-100 transition-opacity">
...
{#if editingPhoto}
	...
	>
		<div class="bg-[var(--card-bg)] rounded-2xl p-6 w-full max-w-md space-y-4 shadow-xl">
			<h3 class="text-lg font-bold text-90">编辑照片信息</h3>
			<div>
				<label class="block text-sm font-medium mb-1 text-75" for="ph-title">照片标题</label>
				<input id="ph-title" bind:value={editPhotoTitle} class="w-full h-10 px-3 rounded-lg border border-[var(--line-divider)] bg-[var(--card-bg)] text-90 focus:outline-none focus:ring-2 focus:ring-[var(--primary)] transition" />
			</div>
			<div>
				<label class="block text-sm font-medium mb-1 text-75" for="ph-desc">照片描述</label>
				<textarea id="ph-desc" bind:value={editPhotoDesc} rows="3" class="w-full px-3 py-2 rounded-lg border border-[var(--line-divider)] bg-[var(--card-bg)] text-90 focus:outline-none focus:ring-2 focus:ring-[var(--primary)] transition resize-y"></textarea>
			</div>
